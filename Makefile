...
# github.io/polyfea-controller-bundle:$VERSION and github.io/polyfea-controller-catalog:$VERSION.
IMAGE_TAG_BASE ?= $(CONTAINER_REGISTRY)/polyfea-controller
...
ifneq ($(strip $(TAGS)),)
TAGS_WITHOUT_SPACES := $(subst $() $(),|SPACE|,$(TAGS))
TAGS_SEPARATED := $(subst $(COMMA),$() $(),$(TAGS_WITHOUT_SPACES))
PARSED_TAGS := $(foreach tag,$(TAGS_SEPARATED),--tag "$(subst |SPACE|,$() $(),$(tag))")
endif

ifneq ($(strip $(LABELS)),)
LABELS_WITHOUT_SPACES := $(subst $() $(),|SPACE|,$(LABELS))
LABELS_SEPARATED := $(subst $(COMMA),$() $(),$(LABELS_WITHOUT_SPACES))
PARSED_LABELS := $(foreach label,$(LABELS_SEPARATED),--label "$(subst |SPACE|,$() $(),$(label))")
endif

ifneq ($(strip $(ANNOTATIONS)),)
ANNOTATIONS_WITHOUT_SPACES := $(subst $() $(),|SPACE|,$(ANNOTATIONS))
ANNOTATIONS_SEPARATED := $(subst $(COMMA),$() $(),$(ANNOTATIONS_WITHOUT_SPACES))
PARSED_ANNOTATIONS := $(foreach annotation,$(ANNOTATIONS_SEPARATED),--annotation "$(subst |SPACE|,$() $(),$(annotation))")
endif
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support in pipeline I want the build CI fail if the docker image is not built
	...
	docker buildx build --push --platform=$(PLATFORMS) $(if $(TAGS),$(PARSED_TAGS),--tag $(IMG)) $(if $(LABELS),$(PARSED_LABELS)) $(if $(ANNOTATIONS),$(PARSED_ANNOTATIONS)) -f Dockerfile.cross .
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	$(KUSTOMIZE) build config/make | kubectl apply -f -

.PHONY: undeploy
undeploy: ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	$(KUSTOMIZE) build config/make | kubectl delete --ignore-not-found=$(ignore-not-found) -f -
...
openapi: ## Download latest openapi spec and generate web api skeleton
	...
	docker run --rm -v $(CURDIR)/web-server/:/local openapitools/openapi-generator-cli:2.16.0 generate -c /local/scripts/generator-cfg.yaml
